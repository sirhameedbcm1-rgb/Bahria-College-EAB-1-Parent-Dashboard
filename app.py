...
st.markdown("""
    ...
    .stMetric { background-color: white; padding: 18px; border-radius: 12px; border: 2px solid #003366; }
    .stExpander { background-color: white; border: 1px solid #003366; border-radius: 10px; }
    </style>
...
with c1: st.metric("Total Students", f"{total_students:,}")
with c2: st.metric("Total Unique Parents", f"{total_parents:,}")
with c3: st.metric("Average Children per Parent", f"{total_students/total_parents:.2f}")
with c4: st.metric("Classes", df["New Class"].nunique() if "New Class" in df.columns else "N/A")
